...
import com.general.component.log.filter.CachingSupportServletRequestWrapper;
...
public class HttpLogResolver implements LogResolver {
    ...
    public HttpLogMessage beforeHandle() throws Exception {
        ...
        if (showRegion) {
            try {
                String region = HostInfo.getSearcherInstance()
                        .search(HostInfo.getIpAddr(requestWrapper));
                httpLogMessage.setRegion(region);
            } catch (Exception e) {
                log.error("HttpLogResolver-ipv6暂时不支持解析地址 ipv6:{}", HostInfo.getIpAddr(requestWrapper));
            }
        }
