package com.general.component.common.regex.constants;
 ...
 */
public class RegexBaseConstant {


    /**
     * 数字与字符组合
     */
    public static Pattern NUM_CHARACTER = Pattern.compile("^[A-Za-z\\d]+$");

    /**
     * 特殊字符
     */
    public static Pattern SPECIAL_CHARACTER = Pattern.compile("^((?=[\\x21-\\x7e]+)[^A-Za-z0-9])x21-\\x7e]+\\)[^A-Za-z0-9]\\)");


    /**
     * 部署IP及端口格式
     */
    public static Pattern IP_PORT = Pattern.compile("^$|[\\\\]\\\\[0-9a-fA-F:]{1}[\\\\]\\\\[.0-9a-fA-F:;]{0,9998}[\\\\]\\\\[0-9a-fA-F:]{1}");


    /**
     * 手机号
     */
    public static Pattern PHONE = Pattern.compile("^$|^1[0-9][0-9]\\\\d{8}$|^(5|6|8|9)[0-9]{7}$");


    /**
     * 护照
     */
    public static Pattern PASSPORT = Pattern.compile("^([EK]\\d{8}|(SE|DE|PE|MA)\\d{7})$");
