...
public class Host {
     ...
     * @throws IOException
     */
    private static Searcher getSearcherInstance() throws IOException {
        if (searcher == null) {
            synchronized (Host.class) {
                if (searcher == null) {
                    InputStream url = Host.class.getClassLoader().getResourceAsStream("data_ip2region.xdb");
    ...
    public static String searchRegion(String ip) throws Exception {
        return getSearcherInstance().search(ip);
    }
